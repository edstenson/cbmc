...
int irept::compare(const irept &i) const
{
  ...
  const auto size = get_sub().size(), i_size = i.get_sub().size();
...

  {
    ...
    it2 = i.get_named_sub().begin();
        ...
        it2 != i.get_named_sub().end();) // no iterator increments
    {
...
std::size_t irept::hash() const
{
  ...
  const std::size_t non_comment_named_sub_count =
    number_of_non_comments(named_sub);

  result = hash_finalize(result, sub.size() + non_comment_named_sub_count);
